async function addCard(bot, chatId, collection) {
    return new Promise(async (res, rej) => {
        try {
            ...
            let buyer = await collection.find({ chatId: ID }).toArray();
            let leftCard = buyer[0][filed];
            leftCard = leftCard + amount;

            let updateStatus = await collection.updateOne({ chatId: ID }, { $set: { [filed]: leftCard } });
            if (updateStatus.modifiedCount) {
                ...
                await bot.sendMessage(chatId, `👉card ပို့ချင်း MA အောင်မြင်ပါသည်`);
